...
{
    // Determine the maximum width needed for any line in the content, plus the title
    int width = 0;
    ...
    // Prints raw text 
    void rawPrint(const std::string &text)
    {
        std::cout << text << "\n";
    }
 ...
 | |__| |___) | . \   ____) | |__| | |__| | |____| |\  |_| || |__| | |  | |  | |   
  \____/|____/|_|\_\ |_____/ \____/ \____/|______|_| \_|_____\_____|_|  |_|  |_|  
        ...
        rawPrint("+-----------------------------------------------------------+\n");
        std::cout << BLINK "1. Enter Dungeon\n2. About\n3. Upgrade\n4. Exit\n" RESET_COLOR;       
        rawPrint("+-----------------------------------------------------------+");
    }
    ...
    {
        clear();
        drawBox("Dungeon Entrance", {RED_COLOR "Behold, the entrance to the Abyss!    ",
                           ...
                           const std::string &enemyName, int enemyHP)
    {
        ...
        rawPrint(GREEN_COLOR + playerName + " HP: " + std::to_string(playerHP) + RESET_COLOR);
        rawPrint(RED_COLOR + enemyName + " HP: " + std::to_string(enemyHP) + RESET_COLOR);
    ...
    {
        clear();
        rawPrint(BLINK YELLOW_COLOR "==================== POWER UP ====================" RESET_COLOR);
