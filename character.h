...
// Build a player class that inherits from Character ; name, max HP, attack power, level, and experience
class PlayerClass : public Character {
    ...
    public:
        PlayerClass(const std::string& name, int hp, int attack)
            : Character(name, hp, attack), level(1), experience(0), upgradePoints(5) {} // Default points set to 5
        ...
        void displayStats() const override {
            std::cout << "Player: " << name << " | HP: " << currentHP << "/" << maxHP
                      ...
                      << " | EXP: " << experience << "/100 | Upgrade Points: " << upgradePoints << std::endl;
        }
        ...
        void addUpgradePoints() {
            ...
            std::cout << "You have gained " << additionalPoints << " upgrade points!" << std::endl;
        }
    };
    

// Build a monster class that inherits from Character ; name, max HP, and attack power
