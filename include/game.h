...
class Game {
    ...
    public:
        ...
        // This function manages the main menu loop, allowing the player to select an
        // option (start game, show about, upgrade, or exit).
        void start() {
            int choice;
            do {
    ...
    private:
        ...
        // This function simulates the combat between the player and the enemy,
        // processing player and enemy actions.
        void combat() {
            srand(
                static_cast<unsigned int>(time(nullptr))); // Seed random number generator

            while (player.isAlive() && enemy.isAlive()) {
                renderer.renderCombatScene(player.getName(), player.getHP(),
                                           enemy.getName(),
                                           enemy.getHP()); // Render combat scene

                std::cout << "\n1. Attack\n2. Defend\nChoose action: "; // Display combat
                                                                        // options
                int choice;
                std::cin >> choice;
                ...
                if (choice == 1) {
                    ...
                    renderer.print(player.getName() + " attacks " + enemy.getName() +
                ...
                if (enemy.isAlive()) {
                    int enemyDamage =
                        rand() % 10 + 3; // Random enemy damage between 3 and 12
                    player.takeDamage(enemyDamage); // Apply damage to player
                    renderer.print(enemy.getName() + " attacks back for " +
                                   std::to_string(enemyDamage) + " damage!");
                }
        ...
        // This function pauses the game and waits for the player to press enter
        // before continuing.
        void waitForEnter() {
        ...
        void upgrade() {
            int upgradeChoice;
            do {
                renderer.renderUpgradeScreen(); // Render the upgrade screen with
                                                // thematic style
