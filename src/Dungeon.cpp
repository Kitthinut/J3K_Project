...
Dungeon::Dungeon(): currentTurn(Player) {
    if (!backgroundTexture.loadFromFile("assets/background/Dungeon.png")) {
        std::cout << "Failed to load dungeon background!" << std::endl;
    }
    ...
    playerSkills = {SkillDB::FireBall, SkillDB::IceSpike, SkillDB::Heal, SkillDB::Thunder,
                    SkillDB::WindSlash};

    // --- Boss image ---
    if (!bossTexture.loadFromFile("assets/boss_beta_test/Goku_Drip_Entity.png")) {
    ...
    if (event.type == sf::Event::KeyPressed) {
        ...
        switch (phase) {
            case BattlePhase::PlayerPick:
                ...
                if (event.key.code == sf::Keyboard::E && selectedSkillIndex != -1) {
...
void Dungeon::render(sf::RenderWindow &window) {
    ...
    // Opening animation
    if (phase == BattlePhase::Opening) {
        drawCenteredBoxWithText(window, "A powerful foe appears!", 700, 100, 960, 440,
                                font, 40, sf::Color(0, 0, 0, 220), sf::Color::Red);
        drawCenteredBoxWithText(window, "Boss: Goku Drip Entity", 700, 80, 960, 540, font,
                                36, sf::Color(0, 0, 0, 200), sf::Color::Yellow);
        window.draw(bossSprite);
        return;
    }
    ...
    if (phase == BattlePhase::PlayerPick) {
        ...
        if (selectedSkillIndex != -1) {
            std::string msg = "Player picked \"" + playerSkills[selectedSkillIndex].name +
                              "\". Press [E] to confirm.";
            drawCenteredBoxWithText(window, msg, 1000, 60, 960, 870, font, 28,
                                    sf::Color(0, 0, 0, 200), sf::Color::Yellow);
        } else {
            drawCenteredBoxWithText(window, "Press [1-5] to select skill", 800, 60, 960,
                                    900, font, 28, sf::Color(0, 0, 0, 200),
                                    sf::Color::Cyan);
        }
    } else if (phase == BattlePhase::PlayerConfirm) {
        // Show selected skill and confirm prompt
        if (selectedSkillIndex != -1) {
            std::string msg = "Selected: " + playerSkills[selectedSkillIndex].name +
                              ". Press [E] to confirm.";
        ...
        drawCenteredBoxWithText(window, "Boss is choosing...", 800, 60, 960, 800, font,
                                28, sf::Color(0, 0, 0, 200), sf::Color::Red);
    ...
    //     drawCenteredBoxWithText(window, lastActionMessage, 1000, 60, 960, 700, font,
    //     28,
...
void Dungeon::setPlayerSkills(const std::vector<Skill> &skills) { playerSkills = skills; }