...
Upgrade::Upgrade(const sf::Font &font, const sf::Vector2f &size,
    ...
    float offset_y      = this->position.y + PADDING;

    points.setFont(font);
    points.setCharacterSize(24);
    points.setPosition(offset_x, offset_y);
    points.setFillColor(sf::Color::White);

    for (int i = 0; i < choices.size(); ++i) {
        float new_offset_y = offset_y + ((i + 1) * 40);

        sf::Text text(choices[i], font, 24);
        text.setPosition(offset_x, new_offset_y);
        text.setFillColor(sf::Color::White);
        texts.push_back(text);

        createButton('-', sf::Color::Red, minus_positon, new_offset_y);
        createButton('+', sf::Color(70, 187, 31, 255), plus_positon, new_offset_y);
    }
}
...
void Upgrade::update(const int point, const Volume &hp, const Volume &mana,
                     const int atk, const int def) {
    points.setString("Stats Points: " + std::to_string(point) + " left");
    texts[0].setString(choices[0] + std::to_string(hp.current) + " / " +
                       std::to_string(hp.max));
    texts[1].setString(choices[1] + std::to_string(mana.current) + " / " +
                       std::to_string(mana.max));
