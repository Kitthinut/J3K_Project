...
void introduction(sf::RenderWindow &window) {
    ...
    if (!font.loadFromFile("C:/Windows/Fonts/CONSOLA.ttf")) {
        std::cerr << "Failed to load font Consolas\n";
        return;
    }
...
void show_credits(sf::RenderWindow &window) {
    sf::Font font;
    if (!font.loadFromFile("assets/fonts/consola.ttf")) {
        std::cerr << "Failed to load font: assets/fonts/consola.ttf\n";
        return;
    }
    ...
    std::vector<std::string> credits = {
        ...
        "Story & Design: You and your sleepless nights ;-;",
        "Programming: Also you (obviously)",
        "Art & Assets: Placeholder Power >_<",
        "",
        "Special Thanks to:",
        "SFML, caffeine, and engineering pain",
        "",
        "Thanks for playing! T_T",
